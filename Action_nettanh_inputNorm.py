...
video_fea_root = '/home/xyj/usr/local/data/ucf-101/flow-feature/ucf-split'+str(split)+'-resize256-crop224-pool5-feature.h5'
...
center_file = '/home/xyj/usr/local/data/ucf-101/centers/action_flow_resize256_crop224_split'+str(split)+'InNorm'+str(input_norm)+'_'+'d'+str(image_filter)+'_k'+str(K_centers)+'.pkl'
(init_w,init_b,init_centers) = CenterUtil.get_action_centers(center_file, hf, used_kmeans_list, image_filter, K_centers,split=split,input_norm=input_norm)
...
seq_vlad = SeqVLADAction(K_centers,image_filter,init_w,init_b,init_centers,inner_init='glorot_uniform',L2_normalize=input_norm)(seq_img_in)
...
export_path = '/home/xyj/usr/local/saved_model/'+'action_split'+str(split)+'/K'+str(K_centers)+'_t'+str(max_timesteps)+'_b'+str(batch_size)+'_lr'+str(lr)+'_d'+str(dropout)+'_l2norm'+str(input_norm) # where to save the exported graph
...
with sess.as_default():
	...
	saver.restore(sess, export_path+'/iter20_L1.06872611127_A0.811827434342.ckpt')
	...
	if(evaluate_step):
		...
		for batch_idx, sample in enumerate(test_v_list):
			...
			output_root = '/home/xyj/usr/local/predict_result/flow_split'+str(split)+'/K'+str(K_centers)+'_d'+str(dropout)+'_lr'+str(lr)+'iter20_L1.068_A0.8118'
